...
describe('NotebookTests', () => {
	...
	describe('Get a Users Notebooks', () => {
		it('This function should return al user notebooks', async () => {
			const result = await notebookService.getUserNotebooks('testUserID');
			expect(result).toStrictEqual([]);
	...
	describe('Get notebook', () => {
		it('This function should return al user notebooks(Only one notebook in this case)', async () => {
			const result = await notebookService.getNotebook('testNotebook');
			...
			expect(result.notebookId).toBe('testNotebook');
